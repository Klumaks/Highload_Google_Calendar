...
Пользователь открывает календарь почти каждый день для просмотра планов и синхронизации (6 дней из 7), но создаёт события 4–6 дней в месяц. Просмотр календаря не создаёт HTTP-запросов, т.к. данные кэшированы на устройстве (продуктовое решение «Синхронизация между устройствами»). Уведомления доставляются асинхронно через брокер сообщений и не создают входящих запросов.
